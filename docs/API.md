...
**`hotkey` is Mac-only.** It is stored per user, not per device, and its values are Mac keys. The Windows client (`windows/`) keeps its own hotkey locally (`rightCtrl` | `rightAlt`) and never applies this field; when it `PUT`s a mode or language change it sends `hotkey` and `llmModel` exactly as it last received them from `GET /v1/me`, and it sends no `PUT` at all until a `GET /v1/me` has succeeded. Any future non-Mac client must do the same, or changing Mode on a PC resets the user's Mac hotkey to `fn`.
